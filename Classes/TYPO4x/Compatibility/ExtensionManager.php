    ...
    // Gets the installed extension
			...
			if (!t3lib_extMgm::isLoaded($extensionKey)) {
				if (!t3lib_extMgm::isLocalconfWritable()) {
				  $message = t3lib_div::makeInstance('t3lib_FlashMessage', 
				    $GLOBALS['LANG']->getLL('ext_import_p_localconf'), 
				    '', 
        ...
        // Adds the extension to the list
          ...
          tx_em_Tools::refreshGlobalExtList();
			...
			$message = t3lib_div::makeInstance('t3lib_FlashMessage', 		
			  $GLOBALS['LANG']->getLL('ext_import_ext_not_loaded'), 
			  '', 
			  t3lib_FlashMessage::ERROR
			);
      t3lib_FlashMessageQueue::addMessage($message);
    ...
    if ($extensionKey === NULL) {
      ...
      if ($newInstalledExtensions != -1) {
        ...
        t3lib_extMgm::removeCacheFiles();
  			...
  			$message = t3lib_div::makeInstance('t3lib_FlashMessage', 
  			  Tx_Extbase_Utility_Localization::translate('error.extensionHasDependencies', 'sav_library_kickstarter'), 
  			  '', 
  			  t3lib_FlashMessage::ERROR
  			);
        t3lib_FlashMessageQueue::addMessage($message);
