...
class Tx_SavLibraryKickstarter_Core_Parser_SyntaxTree_ObjectAccessorNode extends Tx_Fluid_Core_Parser_SyntaxTree_ObjectAccessorNode {
    ...
    $objectPath = $this->objectPath;
		if (preg_match('/^(?P<Start>[^{]*)(?P<ObjectAccessor>{.*})(?P<End>[^}]*)$/', $objectPath, $matchedVariables) > 0) {
      ...
      $objectPath = $matchedVariables['Start'] . $parsedTemplate->render($this->renderingContext) . $matchedVariables['End'];
    }
